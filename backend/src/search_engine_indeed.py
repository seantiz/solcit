...
class JobSearchEngine:
    def __init__(self):
        ...
        self.fetch_job_listings = Indeed()
        ...
        for job in jobs:
            ...
            if existing_job is None:
                c.execute('''INSERT INTO jobs 
                             ...
                             VALUES (?,?,?,?,?,?,?,?,?,?)''',
                          (uniqueid, title, company, location, salary, jobkey, fetched_date, 0, 0, 'indeed'))
                new_jobs += 1
            else:
...
class Indeed:
    ...
    def set_indeed_url(self, keywords, location, max_pages=10):
        encoded_keywords = urllib.parse.quote_plus(keywords)
        base_url = f"https://it.indeed.com/jobs?q={encoded_keywords}"
        if location:
            encoded_location = urllib.parse.quote_plus(location)
            base_url += f"&l={encoded_location}"
        return self.push_indeed_pages(base_url, keywords, max_pages)

    def push_indeed_pages(self, base_url, keywords, max_pages):
        all_jobs = []
        for page in range(max_pages):
            start = page * 10
            url = f"{base_url}&start={start}"
            logger.info(f"Fetching Indeed page {page + 1}...")
            html_content = self.bot_facade(url, keywords)
            if not html_content:
                logger.error(f"Failed to fetch content for Indeed page {page + 1}")
                break

            job_listings = self.parse_indeed_pages(html_content)
            if not job_listings:
    ...
    def parse_indeed_pages(self, html_content):
        soup = BeautifulSoup(html_content, 'html.parser')
        script_tags = soup.find_all('script', type='text/javascript')
        
        for script in script_tags:
            if 'window.mosaic.providerData["mosaic-provider-jobcards"]' in script.string:
                ...
                if match:
                    ...
                    return job_data.get('metaData', {}).get('mosaicProviderJobCardsModel', {}).get('results', [])
        
        return []

    def fetch_jobs(self, keywords, location, max_pages=10):
        # Public method for JobSearchEngines()'s call
        return self.set_indeed_url(keywords, location, max_pages)
...
def main(db_path, keywords, location):
    ...
    conn = sqlite3.connect(db_path)
    new_indeed_jobs = search_engine.push_listings_to_db(conn, results)
    
    logger.info(f"Added {new_indeed_jobs} new Indeed jobs to the database.")
